...
CHANCE_TO_START_ALIVE = 80 #def: 65
DEATH_LIMIT = 3
BIRTH_LIMIT = 4
STEPS_NUMBER = 2
MIN_ROOM_SIZE = 6
...
def unblockTunnels(mapToUse, roomTiles, tunnelTiles):
    for x in range(MAP_WIDTH):
        for y in range(MAP_HEIGHT):
            if mapToUse[x][y].lava and (x,y) in tunnelTiles and not (x, y) in roomTiles:
                mapToUse[x][y].lava = False
    return mapToUse
...
def createWater(mapToUse): #, roomTiles, tunnelTiles, unchasmable):
    newMap = copy.deepcopy(mapToUse)
    for x in range(MAP_WIDTH):
        for y in range(MAP_HEIGHT):
            if randint(1, 100) < CHANCE_TO_START_ALIVE:
                #mapToUse[x][y].baseFg = colors.lighter_grey
                #mapToUse[x][y].baseBg = colors.dark_sepia
                newMap[x][y].water = False
            else:
                newMap[x][y].water = True
    #update(newMap)
    for loop in range(STEPS_NUMBER):
        newMap = doStep(newMap)
        #update(newMap)
    newMap = unblockTunnels(newMap, roomTiles, tunnelTiles)
    return newMap
    
def makeMap():
    global myMap, rooms, roomTiles, tunnelTiles, unchasmable, firstX, firstY, lastX, lastY
    
    myMap, tunnelTiles, roomTiles, rooms = tunneling.makeTunnelMap(returnTunTiles = True)
    myMap = createWater(myMap)
    return myMap
    ...
    for x in range(-1, 2):
        for y in range(-1, 2):
            if x == 0 and y == 0:
                ...
                otherX = startX + x
                otherY = startY + y
                if not mapToUse[otherX][otherY].water:
                    count += 1
                    found = True
                    if stopAtFirst:
                        break
    ...
    for x in range(MAP_WIDTH):
        for y in range(MAP_HEIGHT):
            try:
                ...
                elif mapToUse[x][y].water:
                    root.draw_char(x, y, None, bg = colors.azure)
